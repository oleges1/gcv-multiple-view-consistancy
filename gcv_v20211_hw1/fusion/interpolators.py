...
def get_view(
    ...
    pose_i = CameraPose(extrinsics[i]) #from_camera_axes?
...
def pairwise_interpolate_predictions(
    ...
    for idx, point_from_j in enumerate(reprojected_j):
        ...
        distances_to_nearest = np.abs(point_from_j[:2, None] - point_from_j_nns[None, :])
...
def multi_view_interpolate_predictions(
    ...
    n_images = len(images)
    for i, j in tqdm(itertools.product(range(n_images), range(n_images)), total=n_images*n_images):
